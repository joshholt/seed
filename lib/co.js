...
/**
  ...
  *** NOTE ***
  ...
  @param path [path to a dir] -- Co.path.normalize() is your friend
  @param <optional> filter [a regex string]
  @param done [callback function]
  
*/
Co.fs.glob = function(path, filterExpr, done) {
    ...
    Co.collect(filenames, function(filename, done) {
      ...
      Co.reduce(paths, [], function(ret, path, done) {
        if ('string' === typeof path) ret.push(path);
          ...
          if (err) return done(err);
          if (filterExpr && typeof filterExpr === 'function') {
            Co.filter(success, function(path, fun) {
              Co.sys.puts(path);
              fun(null,filterExpr.test(path));
            })(done);
...
Co.prompt = function(done) {
  ...
  try {
    ...
    if (!e || (e.message!=='sdtin already open')) return done(e);
  }
  
  var listener = function(response) {
    process.stdio.removeListener(listener);
    process.stdio.close();
    done(null, response);
  };
  
  process.stdio.addListener('data', listener);
};
